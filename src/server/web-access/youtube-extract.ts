...
// In agentchatbox, YouTube config comes from env (YOUTUBE_PREFERRED_MODEL, YOUTUBE_ENABLED)
// rather than ~/.pi/web-search.json (which is a pi-CLI convention).
...
function loadYouTubeConfig(): YouTubeConfig {
	...
	const envEnabled = process.env.YOUTUBE_ENABLED;
	const envModel = process.env.YOUTUBE_PREFERRED_MODEL;
	cachedConfig = {
		enabled: envEnabled !== undefined ? normalizeEnabled(envEnabled, defaults.enabled) : defaults.enabled,
		preferredModel: envModel !== undefined ? normalizePreferredModel(envModel, defaults.preferredModel) : defaults.preferredModel,
	};
	return cachedConfig;
}
