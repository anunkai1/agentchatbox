...
// Video config comes from env (VIDEO_ENABLED, VIDEO_PREFERRED_MODEL) in agentchatbox.
...
function loadVideoConfig(): VideoConfig {
	...
	const envEnabled = process.env.VIDEO_ENABLED;
	const envModel = process.env.VIDEO_PREFERRED_MODEL;
	const envMax = process.env.VIDEO_MAX_SIZE_MB;
	cachedVideoConfig = {
		enabled: envEnabled !== undefined
			? normalizeEnabled(envEnabled, VIDEO_CONFIG_DEFAULTS.enabled)
			: VIDEO_CONFIG_DEFAULTS.enabled,
		preferredModel: envModel !== undefined
			? normalizePreferredModel(envModel, VIDEO_CONFIG_DEFAULTS.preferredModel)
			: VIDEO_CONFIG_DEFAULTS.preferredModel,
		maxSizeMB: envMax !== undefined
			? normalizeMaxSizeMB(envMax, VIDEO_CONFIG_DEFAULTS.maxSizeMB)
			: VIDEO_CONFIG_DEFAULTS.maxSizeMB,
	};
