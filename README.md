...
Prompt-image input is bounded to 25 MiB per image, 20 images per prompt, and 300 MiB combined. Oversized images are rejected before upload with a reason, and oversized sets are rejected before send. The pi RPC line limit is 448 MiB: enough for about 400 MiB of base64 plus JSON overhead, while remaining below Node's maximum string length. Binary image blocks are removed from browser replay traffic. Legacy inline images also remain subject to the separate WebSocket payload limit; uploaded-file references avoid sending the image bytes through the browser WebSocket.
