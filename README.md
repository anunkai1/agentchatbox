...
Prompt-image input is bounded to 25 MiB per image, 20 images per prompt, and 500 MiB combined. Oversized images are rejected before upload with a reason, and oversized sets are rejected before send. The pi RPC line limit is 768 MiB to allow for base64 expansion; binary image blocks are removed from browser replay traffic.
